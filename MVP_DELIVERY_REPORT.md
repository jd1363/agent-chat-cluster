...
| Phase 4 | 7 Agent 扩容 + 并发安全 + Web Dashboard | ✅ 已验收 | `ACCEPTANCE_STAGE4.md` |
...
- 7 个 Agent（exec01 + ext01~ext06）全部启用并验证；
- Agent cwd 与 allowedPaths 隔离校验；
- round-robin / load / specialist 分配建议；
- 跨平台文件锁（Windows msvcrt + Unix fcntl），保护 tasks.json 并发写入；
- 7 Agent 并发压力测试通过，零 JSON 损坏、零 ID 重复；
...
- 暗色主题单页应用，6 大模块；
- 任务总览、Agent 状态、审计日志、消息总线、成本台账、告警概览；
- Python 标准库 HTTP 服务，7 个 API 路由；
- 30 秒自动刷新。
...
python scripts\test_isolation.py
...
- 任务总数：39
- done：29
- cancelled：7（含历史 failed 转为 cancelled）
- failed：0（历史 failed 已清理为 cancelled）
- pending：0
- 7 个 Agent 全部启用并隔离校验通过
- 并发安全：文件锁保护下的 7 Agent 并发测试零损坏
...
- 阶段验收：
  ...
  - `ACCEPTANCE_STAGE4.md`
- 操作手册：`docs/OPERATOR_RUNBOOK.md`
- 安全说明：`docs/SECURITY_NOTES.md`
- Web Dashboard：`web/dashboard.html` + `web/server.py`（启动 `python web/server.py` → http://localhost:8765）
- 系统化升级预研：
...
Agent Chat Cluster MVP v1（Phase 0-4）已完成最终交付条件：

- 有清晰阶段路线（Phase 0-4 全部验收通过）；
- 有可运行脚本（30+ Python 脚本，纯标准库）；
- 有任务、审计、消息、审批、安全闸；
- 有真实 subagent 验证与 7 Agent 并发验证；
- 有并发安全修复（跨平台文件锁）；
- 有 Web Dashboard 可视化；
- 有交付报告和演示手册。
